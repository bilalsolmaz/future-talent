...
class AgentKonusma(Base):
    ...
    # MutableList: SQLAlchemy'nin in-place liste değişikliklerini (append vb.) dirty olarak işaretlemesi için gerekli.
    # Olmadan: konusma.mesajlar.append(...) sonrası commit() yaparken değişiklik algılanmayabilir.
    mesajlar: Mapped[list] = mapped_column(MutableList.as_mutable(JSON), nullable=False, default=list)
